...
import { SizeService, Size, SizeFormData, ImportRow } from '../../services/size.service';
...
export class SizeComponent implements OnInit {
  ...
  sizeToDelete: Size | null = null;

  // AG Grid column definitions
  columnDefs: ColDef[] = [
    { headerName: 'Size Name', field: 'name', sortable: true, filter: 'agTextColumnFilter', flex: 1, minWidth: 200 },
    {
      headerName: 'Actions',
      field: 'actions',
      sortable: false,
      ...
      cellRenderer: (params: any) => {
        ...
        editBtn.onclick = () => this.openEditSize(params.data);
        ...
        viewBtn.onclick = () => this.viewSize = params.data;
  ...
  actionItems: ActionItem[][] = [
    [
      { label: 'Add New Size', icon: 'add', onClick: () => this.openCreateSize() },
  ...
  handleExportExcel(): void {
    const headers = ['Size Name', 'Created At'];
    const rows = this.sizes.map(s => [
      s.name,
      s.createdAt || ''
    ]);
    ...
    link.setAttribute('download', `sizes_export_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  handleDeleteSize(): void {
    if (!this.sizeToDelete) return;

    const id = this.sizeToDelete.id;
  ...
  onFormChange(): void {
    ...
    this.inlineMatches = this.sizes.filter(s => {
  ...
  checkDuplicate(name: string, excludeId: number | null = null): Size | null {
    if (!name) return null;
    
    const lowerName = name.trim().toLowerCase();
    
    const found = this.sizes.find(s => {
  ...
  handleSaveSize(): void {
    ...
    if (!this.formData.name?.trim()) {
      console.error('Size name is mandatory');
      return;
    }

    const dup = this.checkDuplicate(
      this.formData.name, 
      this.editingSize?.id || null
    );
  ...
  persistSize(): void {
    ...
    if (this.editingSize) {
      ...
      this.sizeService.updateSize(this.editingSize.id!, this.formData).subscribe({
        next: () => {
        ...
        error: (error) => {
          ...
          const index = this.sizes.findIndex(s => s.id === this.editingSize!.id);
          if (index > -1) {
            this.sizes[index] = {
              ...this.sizes[index],
      ...
      // Create new size
      this.sizeService.createSize(this.formData).subscribe({
        next: () => {
        ...
        error: (error) => {
          ...
          const newId = this.sizes.length ? Math.max(...this.sizes.map(s => s.id || 0)) + 1 : 1;
          const newSize: Size = {
            id: newId,
            name: this.formData.name,
            status: this.formData.isActive === 'Y' ? 'Active' : 'Inactive',
  ...
  prepareImportPreview(parsedRows: any[]): void {
    const mapped: ImportRow[] = parsedRows.map((r, idx) => {
      const row = {
        name: r['size name'] || r['name'] || r['size'] || ''
      };
    ...
    const afterDup = mapped.map(item => {
      ...
      const dupAgainst: (number | string)[] = this.sizes
  ...
  commitImport(): void {
    ...
    toMerge.forEach(item => {
      ...
      if (existingId && typeof existingId === 'number') {
        const idx = newSizes.findIndex(s => s.id === existingId);
        if (idx > -1) {
          newSizes[idx] = { 
            ...newSizes[idx], 
            name: item.row.name || newSizes[idx].name
          };
        }
      }
    });
    ...
    let nextId = newSizes.length ? Math.max(...newSizes.map(s => s.id || 0)) + 1 : 1;
