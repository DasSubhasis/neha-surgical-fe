...
import { BrandService, Brand, BrandFormData, ImportRow } from '../../services/brand.service';
...
export class BrandComponent implements OnInit {
  ...
  columnDefs: ColDef[] = [
    { headerName: 'Brand Name', field: 'name', sortable: true, filter: 'agTextColumnFilter', flex: 1, minWidth: 200 },
    {
      headerName: 'Actions',
      field: 'actions',
      sortable: false,
      ...
      cellRenderer: (params: any) => {
        ...
        editBtn.onclick = () => this.openEditBrand(params.data);
        ...
        viewBtn.onclick = () => this.viewBrand = params.data;
  ...
  actionItems: ActionItem[][] = [
    [
      { label: 'Add New Brand', icon: 'add', onClick: () => this.openCreateBrand() },
  ...
  handleExportExcel(): void {
    const headers = ['Brand Name', 'Created At'];
  ...
  onFormChange(): void {
    ...
    this.inlineMatches = this.brands.filter(b => {
      return b.name.toLowerCase().includes(this.formData.name.trim().toLowerCase());
    }).slice(0, 5);
  }
  ...
  checkDuplicate(name: string, excludeId: number | null = null): Brand | null {
    if (!name) return null;
    ...
    const found = this.brands.find(b => {
      ...
      return b.name && b.name.trim().toLowerCase() === lowerName;
    });
  ...
  handleSaveBrand(): void {
    ...
    if (!this.formData.name?.trim()) {
      console.error('Brand name is mandatory');
      return;
    }

    const dup = this.checkDuplicate(
      this.formData.name, 
      this.editingBrand?.id || null
    );
  ...
  persistBrand(): void {
    ...
    if (this.editingBrand) {
      ...
      this.brandService.updateBrand(this.editingBrand.id!, this.formData).subscribe({
        next: () => {
      ...
      this.brandService.createBrand(this.formData).subscribe({
        ...
        error: (error) => {
          ...
          const newId = this.brands.length ? Math.max(...this.brands.map(b => b.id || 0)) + 1 : 1;
  ...
  prepareImportPreview(parsedRows: any[]): void {
    const mapped: ImportRow[] = parsedRows.map((r, idx) => {
      const row = {
        name: r['brand name'] || r['name'] || r['brand'] || ''
      };
    ...
    const afterDup = mapped.map(item => {
      ...
      const dupAgainst: (number | string)[] = this.brands
        .filter(b => lowerName && b.name && b.name.trim().toLowerCase() === lowerName)
        .map(b => b.id!);
  ...
  commitImport(): void {
    ...
    toMerge.forEach(item => {
      ...
      if (existingId && typeof existingId === 'number') {
        const idx = newBrands.findIndex(b => b.id === existingId);
        if (idx > -1) {
          newBrands[idx] = { 
            ...newBrands[idx], 
            name: item.row.name || newBrands[idx].name
          };
        }
      }
    });
    ...
    let nextId = newBrands.length ? Math.max(...newBrands.map(b => b.id || 0)) + 1 : 1;
